...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    //Image bug fixed by writing image instead of images
    const image = product.image;
    const div = document.createElement("div");
    div.classList.add("col-lg-4");
    //Object Destructuring to access count and rate and used them below
    const { count, rate } = product.rating
    div.innerHTML = `<div class="card h-100">
      
      <img class="card-img-top mx-auto" src=${image}></img>
      <div class="card-body text-center">
      <h5>${product.title}</h5>
      <p>Category: ${product.category}</p>
      <p>${count} People rated this product</p>
      <p>Average Rating : ${rate}</p>
      </div>
      <div class="card-footer text-center mx-auto bg-white">
      <h2>Price: $ ${product.price}</h2>
      <button onclick="addToCart(${product.id},${product.price})"  class="btn btn-outline-success">Add to Cart <i class="fas fa-shopping-cart"></i></button>
      <button class="btn btn-outline-danger">Details <i class="fas fa-info-circle"></i></button>
      </div>
      </div>
      `;
...
const addToCart = (id, price) => {
  ...
  //Total Price Bug Fixed By Calling The Function
  updateTotal();
};
...
const updatePrice = (id, value) => {
  ...
  //Fixing amount to 2 decimal place
...
//Total Price Bug Fixed By Calling The Function (Here to add the delivery charge)
updateTotal();
loadProducts();

const displayCart = () => {
  document.getElementById('my-cart').classList.remove('d-none');
}